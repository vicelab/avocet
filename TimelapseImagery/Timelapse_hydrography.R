...
photolist$timeround <- as.POSIXlt(round(as.double(photolist$datetime)/(interval*60))*(interval*60),origin=(as.POSIXct(tz="GMT",'1970-01-01')))

##Subset to photos to daytime images
photolist <- photolist[photolist$brightness > 30, ]

##Save for another day
# save(photolist, file = "C:/Users/ejholmes/Desktop/Timelapse_hydrography/output/photolist.RDATA")
# load("C:/Users/ejholmes/Desktop/Timelapse_hydrography/output/photolist.RDATA")

##set brightness/contrast transform

l1 <- loess(photolist$brightness~c(1:nrow(photolist)), span = .4)
plot(photolist$brightness)

lines(l1["fitted"]$fitted ~ c(1:nrow(photolist)))
lines(rep(mean(photolist$brightness), length(photolist$brightness))~ c(1:nrow(photolist)))

photolist$exposeAdjustLoess <- round(l1["fitted"]$fitted - photolist$brightness,0)
photolist$exposeAdjustFlat <- round(rep(mean(photolist$brightness), length(photolist$brightness)) - photolist$brightness)
...
sensor_num <- 20
start_date <- "2016-12-1"        #format "YYYY-MM-DD"
# end_date <- Sys.Date()            #format "YYYY-MM-DD"
end_date <- "2017-2-17"

##Download Yolo Bypass near Woodland Data
data <- read.table(paste("http://cdec.water.ca.gov/cgi-progs/queryCSV?station_id=", "BTC", "&sensor_num=", sensor_num, 
                        "&dur_code=E&start_date=", start_date, "&end_date=", end_date, "&data_wish=View+CSV+Data", sep=""),
                  header=FALSE, sep=",", skip=2)
btc <- data

colnames(btc) <- c("Date", "Time", "Flow")
btc$Location <- "Sac River at Butte City"
btc$Time <- sprintf("%04d", btc$Time)
btc$datetime <- paste(btc$Date, btc$Time)
btc$datetime <- as.POSIXct(btc$datetime, format = "%Y%m%d %H%M")

level <- btc[,c("datetime", "Location", "Flow")]
...
for(j in 1:nrow(photolist)){
  ...
  grid.arrange(
    ggplot(levsub, aes(x = datetimeround, y = Flow)) + geom_line(color = "grey30", size = 1) + theme_bw() + 
      geom_ribbon(data = levsub[levsub$datetimeround <= photolist$timeround[j],], aes(ymax = Flow, ymin = min(levsub$Flow)), fill = "blue", alpha = .5) +
      geom_line(data = levsub[levsub$datetimeround <= photolist$timeround[j],], aes(x = datetimeround, y = Flow), size = 2) +
      geom_point(data = levsub[levsub$datetimeround == photolist$timeround[j],], aes(x = datetimeround, y = Flow), size = 8) + 
      labs(list(x = "\nDate", y = "Flow\n (CFS)")) +
...
photoComposite <- function(bgcol = "white",
  ...
  if(parallel == TRUE){
    ...
    foreach(k = 1:nrow(photolist)) %dopar%{
      name <- photolist$photo[k]
      print(paste("overlay graph on:", k, "of", nrow(photolist)))
      shell(cmd = paste("convert -size 2592x1456 -brightness-contrast ", photolist$exposeAdjustFlat[k]/brightDenom,"x",contrast, " ", path, '\\photos\\', name,
                        ...
                        if(plotlocation == "northeast"){paste(' rectangle ', 2592 - plotwidth, ",", plotheight + gap, " ", 2592 - gap, ",", gap, '"', 
                                                              " ", path, '\\output\\temp\\', k,".png",
                                                              " -gravity northeast -geometry ", plotwidth, "x", plotheight, "+", 10, "+15 -composite",sep = "")
                        },
                        
                        if(plotlocation == "southwest"){paste(' rectangle ', gap, ",", 1456 - (plotheight + infobarheight), " ", 
                                                              plotwidth + gap, ",", 1456 + - (gap + infobarheight), '"', 
                                                              " ", path, '\\output\\temp\\', k,".png",
                                                              " -gravity southwest -geometry ", plotwidth, "x", plotheight, "+", gap, "+", infobarheight + gap, " -composite",sep = "")
                        },
                        
                        if(plotlocation == "southeast"){paste(' rectangle ', 2592 - plotwidth, ",", 1456 - (plotheight + infobarheight), " ",
                                                              2592 - gap, ",", 1456 + - (gap + infobarheight), '"', 
                                                              " ", path, '\\output\\temp\\', k,".png",
                                                              " -gravity southeast -geometry ", plotwidth, "x", plotheight, "+", gap, "+", infobarheight + gap, " -composite",sep = "")
                        },
                        
                        ##Output composite image  
                        " ", path, '\\output\\composite\\IMG', sprintf("%04d", k), ".JPG", sep = ""))
    }
    stopCluster(clu)
  }
  
  if(parallel == FALSE){
    for(k in 1:nrow(photolist)){
      name <- photolist$photo[k]
      print(paste("overlay graph on:", k, "of", nrow(photolist)))
      shell(cmd = paste("convert -size 2592x1456 -brightness-contrast ", photolist$exposeAdjustFlat[k]/brightDenom,"x", contrast, " ", path, '\\photos\\', name,
...
#setwd("C:/Users/ejholmes/Desktop/Timelapse_hydrography/output/composite")
...
#shell(cmd = paste('ffmpeg -f image2 -r 20 -i  IMG%4d.JPG -s  1920x1080  Willow_bend.mp4')) ##stabilize
#shell(cmd = paste('ffmpeg -f image2 -r 20 -i  IMG%4d.JPG -s 1280x720 Willow_bend_dflicker.mp4'))
